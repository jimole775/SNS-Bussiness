...
(function() {
	...
	//document.body.innerHTML += [
		...
		'<p class="box-p" ng-repeat="paragraph in item.text track by $index">{{ paragraph }}</p>',
		...
		'<button type="button" ng-class="[\'bottom-bar-button\', {\'bottom-bar-button1\':buttons_arr.length===1}, {\'bottom-bar-button2\':buttons_arr.length===2}, {\'bottom-bar-button3\':buttons_arr.length===3}]"',
		...
		'</ul>',
		'<div class="bottom-bar-button-box">',
		'<button type="button" ng-class="[\'bottom-bar-button\',\'bottom-bar-button1\']" ng-click="handleMenuItemClick(contents_arr,\'00\')"> 返回 </button>',
		'</div>',
		'</div>',
		'</div>'
	].join ("");
