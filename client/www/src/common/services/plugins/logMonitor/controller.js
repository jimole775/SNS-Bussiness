...
(function () {
    ...
    document.onkeydown = function(e){
        ...
        if(e.ctrlKey){          //ctrl
            ...
            switch(e.keyCode){
                ...
                case 38:        //向上
                    codeStorageIndex --;
                    if(codeStorageIndex < 0){codeStorageIndex = 0}
                    $consoleRange.val(codeStorage[codeStorageIndex]);
                    break;

                case 40:        //向下
                    codeStorageIndex ++;
                    if(codeStorageIndex > codeStorage.length){codeStorageIndex = codeStorage.length}
                    $consoleRange.val(codeStorage[codeStorageIndex]);
                    break;
     ...
     * 绑定拖拽事件 *
     **************/
    var startX, startY, moveX, moveY, differenceX, differenceY;
    var box_originX = 0;
    var box_originY = 0;
    var box_originWidth = $AppConsole.width();
    var box_originHeight = $AppConsole.height();
    var touchMinHeightSizeFlag = false;
    var touchMinWidthSizeFlag = false;
    $AppConsoleHead.on({
        touchstart: function (e) {
            startX = e.originalEvent.targetTouches[0].clientX;                        //获取点击点的X坐标
            startY = e.originalEvent.targetTouches[0].clientY;                        //获取点击点的Y坐标
            box_originX = $AppConsole.offset().left;                                  //相对于当前窗口X轴的偏移量
            box_originY = $AppConsole.offset().top;                                   //相对于当前窗口Y轴的偏移量
            differenceX = startX - box_originX;                                       //鼠标所能移动的最左端是当前鼠标距div左边距的位置
            differenceY = startY - box_originY;
        },
        touchmove: function (e) {
            e.preventDefault();
            moveX = e.originalEvent.targetTouches[0].clientX;                         //移动过程中X轴的坐标
            moveY = e.originalEvent.targetTouches[0].clientY;                         //移动过程中Y轴的坐标
            if (moveX + (box_originWidth - differenceX) > win.CONSTANT.WINDOW_WIDTH) { moveX = win.CONSTANT.WINDOW_WIDTH;differenceX = box_originWidth; }
            if (moveY + (box_originHeight - differenceY) > win.CONSTANT.WINDOW_HEIGHT) { moveY = win.CONSTANT.WINDOW_HEIGHT;differenceY = box_originHeight; }
            if (moveX < differenceX) { moveX = differenceX = 0 }
            if (moveY < differenceY) { moveY = differenceY = 0 }
            $AppConsole.css({
                "left": moveX - differenceX,
                "top": moveY - differenceY
            })
        }
    });
    ...
    $extendFunTagHook.on({
        ...
        touchmove: function (e) {
            ...
            moveX = e.originalEvent.targetTouches[0].clientX;                         //移动过程中X轴的坐标
            moveY = e.originalEvent.targetTouches[0].clientY;                         //移动过程中Y轴的坐标
            if ($AppConsole.height() < 200){
                touchMinHeightSizeFlag = true;
                return;
            }
            if($AppConsole.width() < 200){
                touchMinWidthSizeFlag = true;
                return;
            }
            if (moveX >= win.CONSTANT.WINDOW_WIDTH) {moveX = win.CONSTANT.WINDOW_WIDTH - 10}
            if (moveX <= 0) {moveX = 0}
            if (moveY >= win.CONSTANT.WINDOW_HEIGHT) {moveY = win.CONSTANT.WINDOW_HEIGHT - 10}
            if (moveY <= 0) {moveY = 0}

	        $AppConsole.css ({
		        "width": box_originWidth + (moveX - differenceX - box_originX),
		        "height": box_originHeight + (moveY - differenceY - box_originY)
	        });
	        resizeHoverButton ();
        },
        touchend: function (e) {
            e.preventDefault();
            if(touchMinHeightSizeFlag){
                $AppConsole.height(200);
                touchMinHeightSizeFlag = false;
            }
            if(touchMinWidthSizeFlag){
                $AppConsole.width(200);
                touchMinWidthSizeFlag = false;
            }
            box_originWidth = $AppConsole.width();                                        //触摸事件完成后，获取盒子的最后高宽，下次再次触摸时，以这个值为初始值；
            box_originHeight = $AppConsole.height();
        }
    });
})();
